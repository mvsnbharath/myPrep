...
/// Peak pay windows double the base rate (e.g., $0.50 → $1.00 per minute).
/// Also handles at-store pausing and waiting orders from direct pickups.
/// </summary>
class Part3_PeakHourMultiplier
{
    public decimal CalculatePayout(
        List<OrderActivity> orderActivities,
        decimal basePayPerMinute,
        List<(TimeSpan Start, TimeSpan End)> peakWindows)
    {
